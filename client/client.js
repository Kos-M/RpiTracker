...
const printConnectionStats = function () {
  return Helper.printConnectionStats(conn_establish_);
};
//setInterval(printConnectionStats, printConnectionStats_Interval);

async function connect() {
  ID = await execCute("hostname -I | awk '{print $1}'").then(
    (result, error) => {
  ...
  async function process_command(command) {
    switch (command) {
      ...
      case Protocol.GET_HDD:
        execCute(
          " df -h / |  gawk -d  '{ print $2}';df -h / |  gawk -d  '{ print $3}';df -h / |  gawk -d  '{ print $4}';df -h / |  gawk -d  '{ print $5}'"
        ).then(
          (result, error) => {
            // WIP
            if (error) Logger(error);
      ...
      case Protocol.DO_SHUTDOWN:
		...
		break;
		case Protocol.DO_HALT:
			execCute("systemctl suspend").then(
			  (result, error) => {},
			  (error) => {
				ws.send(JSON.stringify({ msg: "error", value: error }));
			  }
  ...
  ws.on("message", async function incoming(data) {
    ...
    switch (ans.msg) {
      ...
      case Protocol.Connected:
        ...
        Logger(
          "Connection with server [" + SERVER + ":" + PORT + "] established."
        );
        heartbeat();
        break;
      default:
